...
var RootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		if cmd.Use == "executor" {
			...
			util.UpdateWhitelist(opts.AllowlistVarRun)
		}
		return nil
	},
...
func addKanikoOptionsFlags() {
	...
	RootCmd.PersistentFlags().BoolVarP(&opts.AllowlistVarRun, "whitelist-var-run", "", true, "Ignore /var/run directory when taking image snapshot. Set it to false to preserve /var/run/ in destination image. (Default true).")
