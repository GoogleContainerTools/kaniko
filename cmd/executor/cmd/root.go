...
func addKanikoOptionsFlags() {
	...
	opts.ClientCertificates = make(map[string]string)
	RootCmd.PersistentFlags().VarP(&opts.ClientCertificates, "registry-client-cert", "", "Use the provided client certificate for mutual TLS (mTLS) communication with the given registry. Expected format is 'my.registry.url=/path/to/client/cert,/path/to/client/key")
